...
namespace concepture\yii2handbook\v2\components\queue\beanstalkd\worker;
...
use Pheanstalk\Job;
use concepture\yii2handbook\components\queue\beanstalkd\worker\WorkerInterface;
use concepture\yii2handbook\components\queue\beanstalkd\worker\WorkerStreamService;
use concepture\yii2handbook\v2\components\queue\events\ExecuteJob;
...
abstract class BaseWorker extends BeanstalkController implements WorkerInterface
{
    ...
    const EVENT_EXECUTE_JOB_DELETE = 'beforeExecuteJobDelete';
    const EVENT_EXECUTE_JOB_BURY = 'afterExecuteJobBury';
    ...
    protected function actionStreamDirect(Job $job)
    {
        ...
        $event = new ExecuteJob();
        $event->payload = $payload;
        $event->queueName = static::getTubeName();

        try {
            $this->trigger(self::EVENT_EXECUTE_JOB_DELETE, $event);
