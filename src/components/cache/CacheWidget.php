...
abstract class CacheWidget extends Widget
{
    ...
    public function beforeRun()
    {
        if(Yii::$app instanceof \yii\web\Application) {
            $this->setOptions([
                'currrent_url_hash' => $this->getDynamicElementsService()->getCurrentUlrHash()
            ]);
        }
    ...
    private function getResult($cache = true)
    {
        ...
        if(! $result) {
            ...
            $this->getDynamicElementsService()->clearCallStack();
            $result = $this->getContent();
            if(! $result) {
                $result = ' ';
            }

            $callStack = $this->getDynamicElementsService()->getCallStack();
